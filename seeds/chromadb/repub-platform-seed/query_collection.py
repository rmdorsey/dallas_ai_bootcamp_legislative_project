...
def main():
    ...
    # Determine the number of results to fetch from the config file.
    n_results = cfg.get('search', {}).get('top_k', 3) # Default to 3 if not found

    query_engine = ChromaQuery(cfg)
    
    # Start an interactive loop to accept queries
    while True:
        try:
            query = input(f"Enter your query (or 'exit' to quit): ")
            if query.lower() == 'exit':
                print("Exiting...")
                break
            
            if query:
                query_engine.search(query, n_results)
